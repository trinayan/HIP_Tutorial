...
template <typename T>
__global__ void
triad(T * triad_e, const T * b, const T * c)
{
    const T scalar = 3.0;
    const int i = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;
    triad_e[i] = b[i] + scalar * c[i];
}
...
int main(int argc, char *argv[])
{
    ...
    char * d_a, * d_b, *d_c, *d_d, *d_e,*triad_b,*triad_c,*triad_e;
    ...
    for (unsigned int k = 0; k < NTIMES; k++)
    {
        ...
        if (groups) {
            ...
            if (useFloat)
                ...
                hipLaunchKernelGGL(triad<double>, dim3(ARRAY_SIZE_2/groupSize), dim3(groupSize), 0, 0, (double*)triad_e, (double*)triad_b, (double*)triad_c);
        }
